...
export class AddListingComponent implements OnInit {
  ...
  ngOnInit() {
    this.checkIdentityAndFetchUser();
  }

  // 1. الفحص الفوري عن فتح الصفحة + جلب رقم الهاتف من جدول الـ user
  private checkIdentityAndFetchUser() {
    const token = localStorage.getItem('spa_token');
    if (!token) {
      this.router.navigate(['/login']);
      return;
    ...
    this.http.get<any>(`${environment.apiUrl}/users/me`, { headers }).subscribe({
      next: (user) => {
        // إذا لم يقدم بطاقة الهوية -> توجيهه فوراً لصفحة التسجيل
        if (!user.has_identity_verification) {
          this.router.navigate(['/verify-identity'], { 
            queryParams: { returnTo: '/marketplace/add-listing' } 
          });
          return;
        }

        // إذا كان موثقاً -> سحب رقم الموبايل تلقائياً من قاعدة البيانات للـ Form
        if (user.phone) {
